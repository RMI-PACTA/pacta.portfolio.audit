add_fund_portfolio <- function(portfolio, fund_portfolio, cols_of_funds) {
  ...
  portfolio_no_funds <- portfolio %>% filter(
    !.data$isin %in% fund_portfolio$fund_isin
  )
  ...
  if (nrow(portfolio_no_funds) + length(unique(fund_portfolio$holding_id)) != nrow(portfolio)) {
    stop("Sum of number of rows in fund portfolio and direct holdings not equal to original rows in portfolio.")
  }
