export_audit_graph_json <- function(audit_file__, export_path_full) {
  ...
  indicator <- TRUE
  for (i in seq_along(flags_in_auditfile)) {
    indicator <- indicator & (flags_in_auditfile[i] %in% all_flags)
  }
  if (isFALSE(indicator)) {
    stop("`indicator` is not `TRUE`")
  }

  number_of_isin <- length(audit_file__$holding_id)

  missing_currency <- audit_file__ %>%
    filter(.data$flag == "Missing currency information")

  number_missing_currency <- length(missing_currency$holding_id)

  negative_missing_input <- audit_file__ %>%
    filter(.data$flag == "Negative or missing input value")

  number_negative_missing_input <- length(negative_missing_input$holding_id)

  invalid_input <- audit_file__ %>%
    filter(.data$flag == "Invalid or missing ISIN")

  number_invalid_input <- length(invalid_input$holding_id)

  not_in_financial <- audit_file__ %>%
    filter(.data$flag == "Holding not in financial database")

  number_not_in_financial <- length(not_in_financial$holding_id)

  included_in_analysis <- audit_file__ %>%
    filter(.data$flag == "Included in analysis")

  number_included_in_analysis <- length(included_in_analysis$holding_id)

  all_flags_numbers <- c(
    number_missing_currency,
    number_negative_missing_input,
    number_invalid_input,
    number_not_in_financial,
    number_included_in_analysis
  )
